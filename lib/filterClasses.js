...
/**
 * Regular expression that metadata on a filter should match
 * @type RegExp
 */
Filter.metadataRegExp = /\{(\{[^{}]*\})\}/;
...
Filter.fromText = function(text)
{
  ...
  if(!ret || !ret.subscriptions){
    console.log("Created a falsy filter from: "+text);
  }
...
/**
 * Abstract base class for Metadata-based filters
 * @param {String} text see Filter()
 * @param {String} Key to search for in metadata
 * @param {String} Value needed for key in metadata
 * @param {String} [domains] Domains that the filter is restricted to, e.g. "foo.com|bar.com|~baz.com"
 * @constructor
 * @augments ActiveFilter
 */
function WhitelistMetadataFilter(text, key, value, domains)
{
  //ActiveFilter.call(this, text, domains);
  WhitelistFilter.call(this, text, null, RegExpFilter.typemap.DOCUMENT, false, domains, false, null)

  this.key = key;
  this.value = value;
}
exports.WhitelistMetadataFilter = WhitelistMetadataFilter;

WhitelistMetadataFilter.prototype = 
{
  __proto__: WhitelistFilter.prototype,

  /**
   * @see ActiveFilter.domainSourceIsUpperCase
   */
  domainSourrceIsUpperCase: true,
  ...
  matches: function(location, contentType, docDomain, thirdParty, sitekey, metadata)
  {
    if(metadata &&
       (metadata[this.key] && ! this.value) 
           || (metadata[this.key] && metadata[this.key]===this.value) &&
...
WhitelistMetadataFilter.fromText = function(text)
{
  let blocking = false;
  let origText = text;

  //TODO: find some way to factor this functionality out so not duplicated
 ...
 * @param {Object} [metadata] Metadata for the filter, with key-value pairs (null value means don't care)
 * @constructor
 * @augments ActiveFilter
 */

//TODO: refactor this name
function RegExpFilter(text, regexpSource, contentType, matchCase, domains, thirdParty, sitekeys, metadata)
{
  ...
  //maybe compile regex now
  if ( regexpSource && regexpSource.length >= 2 && regexpSource[0] == "/" && regexpSource[regexpSource.length - 1] == "/")
  {
  ...
  else
  {
    ...
    // Or this may simply not have a regexp filter
...
RegExpFilter.prototype =
{
  ...
  get regexp()
  {
    // Despite this property being cached, the getter is called
    // several times on Safari, due to WebKit bug 132872
    if(! this.regexpSource){
      //this is not a regexp filter
      return undefined;
    }
  ...
  matches: function(location, contentType, docDomain, thirdParty, sitekey, metadata)
  {
	...
	return true;
      ...
      //check metadata mode
      let mode = this.metadata.__mode;
      delete this.metadata.__mode;
      if(mode === "all"){
	for(var elem in this.metadata){
	  if(this.metadata.hasOwnProperty(elem)){
	    if(!metadata[elem] || metadata[elem]!==this.metadata[elem]){
	      return false;
	    }
	  }
	}
	...
	//else operate in "any" mode
	//pass as long as one element from filter data is in input
	for(var elem in this.metadata){
	  if(this.metadata.hasOwnProperty(elem)){
	    if(metadata[elem] && 
	       (! this.metadata[elem] || this.metadata[elem]===metadata[elem])){
	      return true;
	    }
	  }
	}
...
RegExpFilter.fromText = function(text)
{
  ...
  let metadataAllFlag;
  let options;
  let optmatch = (text.indexOf("$") >= 0 ? Filter.optionsRegExp.exec(text) : null);
  if (optmatch)
  {
    options = optmatch[1].toUpperCase().split(",");
    text = optmatch.input.substr(0, optmatch.index);
    for (let option of options)
    {
      ...
      else if (option[0] == "~" && option.substr(1) in RegExpFilter.typeMap)
      {
        ...
        sitekeys = value;
      else if (option == "META_ALL")
        metadataAllFlag = true;
      else
        return new InvalidFilter(origText, "Unknown option " + option.toLowerCase());
    }
  }

  //now parse out any metadata
  let metadatamatch = (text.indexOf("{{") >= 0 ? Filter.metadataRegExp.exec(text) : null);
  if(metadatamatch && metadatamatch[1])
  {
	...
	metadata[__mode]="all";
      ...
      text = text.slice(0, metadatamatch.index);
    }
    catch(e){
      return new InvalidFilter(origText, "Invalid metadata JSON: "+e);
    }
  }
  ...
  try
  {
    if (blocking)
      ...
      return new WhitelistFilter(origText, text, contentType, matchCase, domains, thirdParty, sitekeys, metadata);
  }
  catch (e)
  {
 ...
 * @augments RegExpFilter
 */
function WhitelistFilter(text, regexpSource, contentType, matchCase, domains, thirdParty, sitekeys, metadata)
{
  RegExpFilter.call(this, text, regexpSource, contentType, matchCase, domains, thirdParty, sitekeys, metadata);
}
