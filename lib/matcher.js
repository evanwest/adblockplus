...
Matcher.prototype = {
  ...
  findKeyword: function(filter)
  {
    ...
    //Remove metadata filters
    match = Filter.metadataRegExp.exec(text);
    if(match)
      text = match.input.substr(0, match.index);
   ...
   */
  _checkEntryMatch: function(keyword, location, contentType, docDomain, thirdParty, sitekey, metadata)
  {
    let list = this.filterByKeyword[keyword];
    for (let i = 0; i < list.length; i++)
    {
      let filter = list[i];
      if (filter.matches(location, contentType, docDomain, thirdParty, sitekey, metadata))
        return filter;
    }
    return null;
  },
   ...
   */
  matchesAny: function(location, contentType, docDomain, thirdParty, sitekey, metadata)
  {
    ...
    for (let i = 0, l = candidates.length; i < l; i++)
    {
      ...
      {
        let result = this._checkEntryMatch(substr, location, contentType, docDomain, thirdParty, sitekey, metadata);
        if (result)
          return result;
      }
    }
...
CombinedMatcher.prototype =
{
   ...
   */
  matchesAnyInternal: function(location, contentType, docDomain, thirdParty, sitekey, metadata)
  {
    let candidates = location.toLowerCase().match(/[a-z0-9%]{3,}/g);
    if (candidates === null)
      candidates = [];
    ...
    for (let i = 0, l = candidates.length; i < l; i++)
    {
      ...
      {
        let result = this.whitelist._checkEntryMatch(substr, location, contentType, docDomain, thirdParty, sitekey, metadata);
        if (result)
          return result;
      }
  ...
  matchesAny: function(location, contentType, docDomain, thirdParty, sitekey, metadata)
  {
	...
	key+=(" "+entry+":"+metadata[entry]);
      }
    }
    if (key in this.resultCache)
      return this.resultCache[key];

    let result = this.matchesAnyInternal(location, contentType, docDomain, thirdParty, sitekey, metadata);
